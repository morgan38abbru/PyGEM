    ...
    def get_annual_mb(
        ...
        if self.glacier_area_initial.sum() > 0:
            ...
            if pygem_prms['mb']['include_supra_lakes']:
                ...
                # but only within the overdeepening (bed below terminal moraine).
                if self._lake_growth_rules and np.any(self.supra_lake_coverage > 0):
                    ...
                    # Identify overdeepening bins: same logic as detect_lake_formation_potential.
                    # Find terminus (last bin with ice > 1 m), then walk upstream while bed
                    # stays below the moraine elevation (max of last two ice bins at terminus).
                    terminus_bins = np.where(thick > 1.0)[0]
                    if len(terminus_bins) > 0:
                        terminus_idx = terminus_bins[-1]
                        if terminus_idx < len(bed_h) - 1:
                            moraine_elev = max(bed_h[terminus_idx], bed_h[terminus_idx + 1])
                        else:
                            moraine_elev = bed_h[terminus_idx]
                    ...
                    for bin_idx in np.where(self.supra_lake_coverage > 0)[0]:
                        # Skip bins outside the overdeepening — no growth allowed
