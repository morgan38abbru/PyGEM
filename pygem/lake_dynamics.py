...
class LakeSemiImplicitModel(SemiImplicitModel):
    ...
    **kwargs : passed to SemiImplicitModel (must include water_level)
    moraine_elev : float or None
        Moraine elevation [m a.s.l.], stored for reference and passed
...
class NewLakeSemiImplicitModel(LakeSemiImplicitModel):
    ...
    **kwargs : passed to LakeSemiImplicitModel (must include water_level)
    ...
    def __init__(self, flowlines, initial_seed_bin=None, moraine_elev=None, **kwargs):
        ...
        if initial_seed_bin is not None:
            self._lake_area_m2 = float(fl.widths_m[initial_seed_bin] * fl.dx_meter)
        else:
    ...
    def step(self, dt):
        ...
        # Run parent SIA + MB step with calving disabled, bypassing
        # LakeSemiImplicitModel.step() to go directly to SemiImplicitModel
        was_calving = self.do_calving
        self.do_calving = False
        ...
        for fl in self.fls:
            ...
            # Fill calving bucket
            # ----------------------------------------------------------
            q_calving = self.calving_k * d_used * h * fl.widths_m[calving_front_idx]
            fl.calving_bucket_m3 += q_calving * dt_actual
            self.calving_m3_since_y0 += q_calving * dt_actual
            ...
            if use_empirical:
                ...
                while fl.calving_bucket_m3 >= vol_terminus and current >= 0:
                    ...
                    while (
                        ...
                        self._lake_area_m2 += float(fl.widths_m[earned_bin] * fl.dx_meter)
                        self._seq_idx += 1
                        self._set_next_target(fl)
